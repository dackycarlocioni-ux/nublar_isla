/**
 * Gate d'accesso per l'intero sito — pensato per accesso QR-only.
 *
 * Come funziona:
 * 1. Il link nel QR code è tipo:  https://tuosito.workers.dev/?key=IL_TUO_TOKEN
 ...
 *
 * Variabili da impostare come SECRET sul Worker (non nel wrangler.toml):
 *   wrangler secret put PARTY_TOKEN
 *   wrangler secret put SESSION_SECRET
 * In locale, mettile invece in un file .dev.vars (mai committato).
 *
 * Genera entrambi i valori con qualcosa tipo:
 *   openssl rand -hex 24
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!env.PARTY_TOKEN || !env.SESSION_SECRET) {
      return new Response(
        "Configurazione mancante: imposta i secret PARTY_TOKEN e SESSION_SECRET sul Worker.",
        { status: 500 }
      );
    }

    // 1. Cookie di sessione già valido? Lascia passare verso gli asset statici.
